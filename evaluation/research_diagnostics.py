...
def run_level_b_difficulty_scan(
    ...
    cfo_abs_cycles_per_symbol: float = 0.0,
    phase_noise_std: float = 0.0,
    rho: float = 0.99,
    traditional_cfo_limit: float = 0.001,
) -> dict:
    """扫描 Level B 下传统 baseline 难度，不包含神经网络或 RL。"""

    rows = []
    cfo_abs = abs(float(cfo_abs_cycles_per_symbol))
    phase_std = float(phase_noise_std)
    cfo_limit = float(traditional_cfo_limit)
    acquisition_cfo_limit = max(0.004, cfo_limit)
    has_phase_impairment = str(impairment_profile) != "clean" or cfo_abs > 0.0 or phase_std > 0.0
    for delay in delays:
        for snr_db in snrs:
            for pilot_total in pilot_totals:
                for layout in pilot_layouts:
                    for seed in seeds:
                        signed_cfo = _signed_cfo_for_seed(cfo_abs, int(seed))
                        env = CommunicationEnvironment(
                            CommEnvConfig(
                                ...
                                cfo_cycles_per_symbol=float(signed_cfo),
                                phase_noise_std=float(phase_std),
                                rho=float(rho),
                            )
                        )
                        start = env.reset_episode()
                        if not has_phase_impairment:
                            cir = _estimate_cir_from_known_frame(start.acquisition, int(delay))
                            acquisition_cfo_hat = 0.0
                        else:
                            cir, acquisition_cfo_hat = estimate_acquisition_cir_with_cfo(
                                start.acquisition,
                                int(delay),
                                cfo_limit=float(acquisition_cfo_limit),
                            )
                        ...
                        for frame_index in range(1, int(frames) + 1):
                            ...
                            for method in methods:
                                result = run_traditional_equalizer(
                                    ...
                                    residual_cfo_limit=float(cfo_limit),
                                )
                                ...
                                rows.append(
                                    {
                                        ...
                                        "phase_noise_std": float(phase_std),
                                        "rho": float(rho),
                                        "traditional_cfo_limit": float(cfo_limit),
                                        "acquisition_cfo_limit": float(acquisition_cfo_limit),
    ...
    for row in rows:
        grouped[
            (
                ...
                row.get("cfo_abs_cycles_per_symbol", 0.0),
                row.get("phase_noise_std", 0.0),
                row.get("rho", 0.99),
            )
    ...
    summary = [
        {
            ...
            "cfo_abs_cycles_per_symbol": key[6],
            "phase_noise_std": key[7],
            "rho": key[8],
...
def _signed_cfo_for_seed(cfo_abs: float, seed: int) -> float:
    if float(cfo_abs) == 0.0:
        return 0.0
    return float(cfo_abs) if int(seed) % 2 == 0 else -float(cfo_abs)
