...
def compute_loss(ac, states, known_mask, true_bits, cw=0.0):
    ...
    cond = ac.extract_condition(states)
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="pretrained_v8")
    ...
    save_dir = args.save_dir  # pretrained_v8
    ...
    print("="*65)
    print("  RL4EQ V8 — 多信道混合预训练")
    print("="*65)
    print(f"  参数量: {params}  步数: {args.num_steps}")
    print(f"  每步随机信道 + SNR(-2~12dB)  因果掩码")
    print()

    optimizer = torch.optim.AdamW(ac.parameters(), lr=1e-4, weight_decay=1e-5)
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=args.num_steps, eta_min=5e-6)
    ...
    _ = env.channel._taps.clone()  # placeholder
    ...
    for step in range(1, args.num_steps + 1):
        cw = 0.1 if step > 1000 else 0.0
        for _ in range(3):
            if step == 1:
                first_channel = env.channel._taps.clone()
            change_ch = False
            if step > 400 and step % 50 == 1:
                change_ch = True
            if change_ch:
                ...
                env.channel.set_taps(first_channel)
                env.set_snr(5.0)
        ...
        if step % 250 == 0:
            ...
            with torch.no_grad():
                ...
                cond_v = ac.extract_condition(vs)
                for film_v in ac.film_layers:
                    x = film_v(x, cond_v)
    ...
    print(f"\n{'='*65}")
    print(f"  预训练 V8 完成! ({elapsed:.0f}s)")
