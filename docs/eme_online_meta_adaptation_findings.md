# EME 在线元适配历史记录（已退出当前主线）

> 本文件只保留历史实验记录，不属于当前研究方法。当前主线不设第二层元学习，
> 直接从离线整帧 `BCE_all` 网络进入安全 Contextual Bandit 在线调度；只有动作
> 被多 seed、多 SNR 证实存在跨帧延迟效应时，才考虑安全 Recurrent Double DQN。
