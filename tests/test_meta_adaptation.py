...
def test_training_tail_mode_online_soft_uses_receiver_state_tail():
    from training.curriculum import select_training_tail

    oracle_tail = torch.tensor([1.0 + 0.0j, -1.0 + 0.0j])
    receiver_tail = torch.tensor([-0.25 + 0.0j, 0.75 + 0.0j])

    selected = select_training_tail(
        frame_tail=oracle_tail,
        receiver_tail=receiver_tail,
        mode="online_soft",
    )

    assert torch.equal(selected, receiver_tail)


def test_training_tail_mode_oracle_preserves_frame_tail():
    from training.curriculum import select_training_tail
    ...
    assert torch.equal(selected, oracle_tail)


def test_validation_phase_features_follow_online_tail_state():
    from training.curriculum import select_validation_tail

    oracle_tail = torch.tensor([1.0 + 0.0j, -1.0 + 0.0j])
    receiver_tail = torch.tensor([-0.25 + 0.0j, 0.75 + 0.0j])

    selected = select_validation_tail(
        frame_tail=oracle_tail,
        receiver_tail=receiver_tail,
        sequence_state=True,
    )
...
def test_corrupt_training_tail_is_deterministic_and_can_flip_all_symbols():
    from training.curriculum import corrupt_training_tail

    tail = torch.tensor([1.0 + 0.0j, -1.0 + 0.0j, 1.0 + 0.0j])

    corrupted_a = corrupt_training_tail(tail, flip_probability=1.0, sample_index=7)
    corrupted_b = corrupt_training_tail(tail, flip_probability=1.0, sample_index=7)

    assert torch.equal(corrupted_a, -tail)
...
def test_noisy_cir_augmentation_supports_cuda_generator_device():
    if not torch.cuda.is_available():
        return

    config = load_config("configs/continual_ppo.json")
    trainer = CurriculumTrainer(config, device=torch.device("cuda"))
    cir = torch.tensor([1.0 + 0.0j, 0.2 - 0.1j], device="cuda")

    noisy = trainer._noisy_cir(cir, noise_std=0.05, sample_index=0)

    assert noisy.device.type == "cuda"
    assert torch.isfinite(noisy.real).all()
    assert torch.isfinite(noisy.imag).all()
