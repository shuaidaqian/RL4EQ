...
def test_summary_uses_explicit_eme_main_matrix():
    from evaluation.metrics import summarize_main_matrix

    summary = summarize_main_matrix(
        [
            {"method": "test", "level": "B", "delay": 116, "snr_db": 0.0, "ber_data": 0.4},
            {"method": "test", "level": "B", "delay": 116, "snr_db": 10.0, "ber_data": 0.2},
            {"method": "test", "level": "C", "delay": 116, "snr_db": 0.0, "ber_data": 0.9},
        ],
        main_delays=[116],
        main_snrs=[0.0, 5.0, 10.0, 15.0],
    )

    assert [(item.delay, item.snr_db) for item in summary.per_config] == [
        (116, 0.0),
        (116, 10.0),
    ]
    assert summary.generalization["count"] == 1
