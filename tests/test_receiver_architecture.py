...
def test_unfolded_equalizer_analytic_logit_skip_recovers_identity_bpsk_with_zero_head():
    """解析 logit skip 应给 Offline NN 一个物理 warm-start，而不是从随机 head 学起。"""

    bits = torch.tensor([[0, 1, 1, 0, 1, 0, 0, 1]], dtype=torch.bool)
    symbols = torch.complex(bits.to(torch.float32) * 2.0 - 1.0, torch.zeros_like(bits, dtype=torch.float32))
    rx_iq = torch.stack((symbols.real, symbols.imag), dim=-1)
    cir = torch.zeros(1, 4, dtype=torch.complex64)
    cir[:, 0] = 1.0 + 0.0j
    condition = condition_from_cir(cir, snr_db=10.0)
    region_ids = torch.zeros_like(bits, dtype=torch.long)
    tail = torch.zeros(1, 3, dtype=torch.complex64)
    ...
    assert torch.all(logits[bits] > 5.0)
    assert torch.all(logits[~bits] < -5.0)
...
def test_unfolded_equalizer_phase_correction_rejects_single_bad_pilot_block():
    """长前缀中单个低信噪比 block 异常时不得把整帧旋转到异常相位。"""
    ...
    indices = torch.arange(32, dtype=torch.float32)
    clean = torch.ones(1, 32, dtype=torch.complex64)
    rotated = clean * torch.exp(1j * (0.3 + 2.0 * torch.pi * 0.002 * indices)).unsqueeze(0)
    rx_iq = torch.stack((rotated.real, rotated.imag), dim=-1)
    phase_features = torch.zeros(1, 96)
    for block in range(4):
        phase_features[0, block * 4 + 0] = 0.3
        phase_features[0, block * 4 + 1] = 0.002
    phase_features[0, 0] = 2.5
    phase_features[0, 1] = 0.08
    condition = SimpleNamespace(
